...
REWARD_MEAN = 4.505817036437599
REWARD_STD = 1.5289818529048889

# The values are tuples of (reward_model_architecture, reward_model_checkpoint_path)
REWARD_MODEL_PATHS = {
    "human_labels_Llama3.1_new": (
        "meta-llama/Llama-3.1-8B-Instruct",
        CURRENT_DIR
        / "model_checkpoints/reward_models/GENERAL_MODEL_Llama-3.1-8B-Instruct_DATA_human_LR_1e-5_BC_8_MAXEPOCH_5_TIME_2025-04-03_21-37-56/checkpoint-3800",
    ),
...
def setup_reward_model(
    ...
    """
    rw_tokenizer = AutoTokenizer.from_pretrained(checkpoint_path, use_fast=False)
...
def get_scores(samples: List[str]) -> torch.Tensor:
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(samples), BATCH_SIZE)):
            ...
            scores_list.append(sub_scores["logits"][:, 0])

    scores = torch.cat(scores_list, dim=0)

    # Normalize the scores
    scores = (scores - REWARD_MEAN) / REWARD_STD
    return scores

...
REWARD_MODEL_PATHS = {
    "Llama3.1_base": (
        ...
        CURRENT_DIR
        / "model_checkpoints/reward_models/SPECIFIC_MODEL_Llama-3.1-8B_DATA_qa_le8000_LR_1e-6_BC_32_MAXEPOCH_10_TIME_2025-05-12_13-22-21/checkpoint-700",
    ),
}
BATCH_SIZE = 4
...
def get_scores(samples: List[str]) -> torch.Tensor:
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(samples), BATCH_SIZE)):
            ...
            probabilities_agree = probabilities[:, 1]
            scores_list.append(probabilities_agree)

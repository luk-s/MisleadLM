...
from trlx.pipeline.ppo_pipeline import PPORolloutStorage, GLMPPORolloutStorage
...
class AcceleratePPOTrainer(AccelerateRLTrainer):
    ...
    def loss(self, batch):
        ...
        if self.config.model.model_arch_type == "seq2seq":
            ...
            if self.tokenizer.__class__.__name__ == 'GLMGPT2Tokenizer': 
                max_prompt_length = self.config.train.seq_length - self.config.method.gen_kwargs["max_new_tokens"]
                inputs = self.tokenizer(query_strs, truncation=True, padding=True, max_length=max_prompt_length, return_tensors='pt')
                query_tensors = inputs.input_ids
                inputs = self.tokenizer.build_inputs_for_generation(inputs, targets=response_strs, max_gen_length=self.config.method.gen_kwargs['max_new_tokens']+1)
                ...
                outputs = self.model(tokens, attention_mask=attention_mask, position_ids=position_ids, return_dict=True)
                logits = outputs.logits
                values_pred = outputs.value
                values_pred = values_pred[:, :-1]
                logprobs = logprobs_of_labels(logits[:, :-1, :], tokens[:, 1:])
                
                mask = response_tensors.ne(self.tokenizer.pad_token_id).long().to(self.accelerator.device)
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            padded_samples = self.accelerator.pad_across_processes(
                samples, dim=1, pad_index=self.tokenizer.eos_token_id if "generation_attention_mask" not in batch else self.tokenizer.eop_token_id, pad_first=False
            )
            ...
            outputs = self.tokenizer([i + self.tokenizer.eos_token for i in str_outputs], add_special_tokens=False).input_ids
        ...
        stats = {k: sum([xs[k] for xs in accumulate_stats]) / len(accumulate_stats) for k in stats}
